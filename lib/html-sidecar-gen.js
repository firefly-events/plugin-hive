...
// Allow only relative, anchor, and known-safe schemes for link targets.
// Anything else (javascript:, data:, vbscript:, …) is neutralized to '#'.
function sanitizeUrl(url) {
  const trimmed = String(url).trim();
  if (/^(https?:|mailto:|#|\/|\.{1,2}\/)/i.test(trimmed)) return trimmed;
  // Scheme-less (relative) targets are safe; an explicit unknown scheme is not.
...
function inlineMarkdown(str) {
  // Escape first so raw HTML/script in the source markdown cannot inject into
  // the generated sidecar; markdown markers below are ASCII and survive escaping.
  str = escapeHtml(str);
  // Bold
  ...
  // Links — validate the href scheme so javascript:/data: targets can't execute.
  str = str.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, text, href) => `<a href="${sanitizeUrl(href)}">${text}</a>`);
  return str;
}
