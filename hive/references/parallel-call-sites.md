...
- [`agent-teams-guide.md`](agent-teams-guide.md) — TeamCreate mechanics and limitations
...
(via `TeamCreate`, the cmux pane loop, the sessions API, or the sandcastle
...
external driver, planning teams composed in a single `TeamCreate` call,
...
|---|---|---|---|
| `execute:team` | [`skills/execute/SKILL.md`](../../skills/execute/SKILL.md) step 6 → [`references/team-execution.md`](../../skills/execute/references/team-execution.md) | Single `TeamCreate` fans depth-0 unblocked stories into one team; one pane per story | per-story (planner-emitted) |
...
| `planning-routing:mixed-team` | [`skills/hive/skills/planning-routing/SKILL.md`](../../skills/hive/skills/planning-routing/SKILL.md) | Direct-routed personas land in one `TeamCreate`; Codex-routed personas land in separate `agent-spawn` → `codex-invoke` panes | `read-only` (planning team produces docs, not code) | The personas are one planning team, not N independent stories. |
...
| [`hive/references/agent-teams-guide.md`](agent-teams-guide.md) | `TeamCreate` mechanics + the rule that teammates cannot spawn nested teams |
