...
(step 6c). Each session corresponds to a single story execution. The registry lives at
`${HIVE_STATE_DIR}/sessions/index.yaml` and is initialized by the session-registry bootstrap skill.

## Storage Path

```
${HIVE_STATE_DIR}/sessions/index.yaml
```
...
- `skills/hive/skills/session-registry/SKILL.md` — bootstrap command that creates `${HIVE_STATE_DIR}/sessions/index.yaml`
- `skills/execute/SKILL.md` — step 6c creates and updates session records
