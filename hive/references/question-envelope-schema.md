...
Two independent mutation paths on a `pending` envelope — answering (terminal) and renewing
(non-terminal, extends validity):
...
answered -> <terminal>     (no further mutation; a new phase gets a new envelope)
```
...
returns the answers, and the skill proceeds to its next phase (which may write a new
envelope).
...
- **The Hive skill / gateway** writes `id`, `skill`, `phase`, `provenance`, the initial
  ...
  It never writes `answer` or mutates `status` to `answered`.
