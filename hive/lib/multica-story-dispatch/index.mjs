...
export function serializeStoryBrief(story) {
  ...
  sections.push(
    `**Push constraint (advisory):** Enforcement is the workflow-phase verifier — not this footer.\n` +
    `- Permitted push target for this team cell: \`feat/${epicId}\` (firefly origin).\n` +
    `- \`agent/developer/<task>\` orphan branches are forbidden.\n` +
    `- Commit author: \`hive-worker <hive-worker@noreply.github.com>\`.`,
  );

  sections.push(
    `---\n_Generated by hive multica-story-dispatch — story ${story?.id ?? ''} in epic ${epicId}_`,
  );
