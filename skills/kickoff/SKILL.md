...
- A `project_type` value is **valid** only when it is exactly one of `framework`, `consumer-app`, or `service`. If the user supplies anything else, re-prompt with the allowed values until one is chosen; do not persist an out-of-set value. **Headless:** an out-of-set answer means the `project-classification` envelope was answered but invalid — write a **new** envelope at `project-classification-round-2` (same round-suffix convention as design's touchpoints, `hive/references/wireframe-protocol.md`) asking only for `project_type` again, rather than re-using the `project-classification` phase id, which would just re-match the already-answered (invalid) envelope.
