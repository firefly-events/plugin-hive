...
`hive/lib/runtime_mode.{py,js}`'s `detect_interactive_mode()` resolves headless,
route it through `hive/lib/question_gateway.{py,js}`'s `ask_or_emit()` instead — see
...
- A `project_type` value is **valid** only when it is exactly one of `framework`, `consumer-app`, or `service`. If the user supplies anything else, re-prompt with the allowed values until one is chosen; do not persist an out-of-set value. **Headless:** an out-of-set answer means the `project-classification` envelope was answered but invalid — write a **new** envelope at `project-classification-round-2` (same round-suffix convention as design's touchpoints, `hive/references/wireframe-protocol.md`) asking only for `project_type` again, rather than re-using the `project-classification` phase id, which would just re-match the already-answered (invalid) envelope. **Persist `has_ui` immediately upon consuming the round-1 envelope**, even though `project_type` still needs a retry round — the gateway deletes a consumed envelope once its answers are extracted (see `hive/references/question-envelope-schema.md`), so a valid `has_ui` answer must be written to `.pHive/project-profile.yaml` right away rather than held in memory waiting for `project_type` to also become valid; a fresh round-2 process has no other way to recover it.
...
- If the user chooses `custom`, require a non-empty shell command and persist it to `ship_target.command`; if no command is provided, re-prompt until one is provided or the user chooses a non-custom kind. **Headless:** the custom command is collected up front in the same `1b` envelope (see kickoff-protocol.md's Headless note under Step 1b); if it's still missing on resume, write a new envelope at `1b-round-2` asking only for the command, rather than re-using phase `1b`. **Persist `ship_target.kind` and `ship_target.notes` immediately** upon consuming the round-1 envelope, even though `custom_command` still needs a retry round — the gateway deletes a consumed envelope once its answers are extracted, so `kind`/`notes` must be written to `.pHive/project-profile.yaml` right away rather than held pending the command also becoming valid.
