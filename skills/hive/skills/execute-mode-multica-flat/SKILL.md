---
name: execute-mode-multica-flat
description: Legacy single-developer Multica dispatch path. One Multica issue per Hive story; the assigned `developer` agent runs the whole story internally. Kept for the parallel-run window (tce-15); removal tracked by tce-16.
---

# Hive Mode — Multica (Flat / Single-Developer)

Atomic skill, NOT inline `/execute` prose. Legacy single-developer dispatch path, preserved verbatim from the pre-tce-8 `execute-mode-multica` skill. Selected when `HIVE_MULTICA_SUBMODE=flat` (or `execution.multica.submode: flat` in config). Default during the tce-15 parallel-run window; removal tracked by tce-16.

Multica flat mode treats each Hive story as one Multica issue assigned to the bootstrapped `developer` agent. Multica owns the internal task work directory and task execution after assignment. Hive owns dispatch, polling, episode markers, and returning a depth summary to `/execute`.

## Invocation contract

Called once per parent workflow when `mode_decision == multica` AND `multica_submode == flat` was resolved by the step-6e flag shim. The trigger is either:

- `HIVE_EXECUTION_MODE=multica` with `HIVE_MULTICA_SUBMODE=flat` (or absent/default)
- root `hive.config.yaml` with `execution.mode: multica` and `execution.multica.submode: flat` (or absent/default)
...
execution:
  ...
  multica:
    submode: flat                    # flat (legacy) | cell (new); default flat during parallel-run window
...
| Rule | Enforcement |
|---|---|
| Atomic skill, not inline `/execute` prose | This file owns the Multica flat lifecycle for selected sub-mode |
...
| Legacy path — removal tracked | tce-16 tracks removal after one release cycle post-tce-15 parallel-run |
