---
name: design-review-dispatch
description: Resolve /design-review mode selection from caller-supplied environment, config, workflow, and arguments. Structural mirror of design-dispatch/SKILL.md — Step 0/1/1.5/2 shape is identical; only the varName ('HIVE_DESIGN_REVIEW_MODE') and skill-path references differ. Flag pass-through: --skip and --artifact-target flow unchanged from caller to resolved mode atom.
---

# Hive Design Review Dispatch

<!-- Structural mirror anchor: skills/hive/skills/design-dispatch/SKILL.md -->
<!-- Any change to the Step 0/1/1.5/2 logic contract in design-dispatch must be evaluated for back-propagation here. -->

Atomic skill, NOT inline `/design-review` prose. It resolves the pre-execution dispatch layer for `/design-review` and returns the mode and runner decisions the caller switches on. It inherits the caller's model and does not choose or override it.
...
- `execution_mode`:
  - Resolved via `hive/lib/mode-resolver.mjs` — call `resolveMode('HIVE_DESIGN_REVIEW_MODE', ctx)` where `ctx` is built from the current environment and root config.
  - Call contract: `const { decision, sources } = resolveMode('HIVE_DESIGN_REVIEW_MODE', { env, rootConfig, shippedBaseline, skillOverride })`
  - Returns `{ decision, sources }` where `sources` contains only the winning tier key (e.g. `{ env: 'HIVE_DESIGN_REVIEW_MODE=sandcastle' }` or `{ root_config: 'execution.mode=multica' }` or `{ default: 'auto' }`).
...
Env wins over config when both are set for the same field (e.g. `HIVE_DESIGN_REVIEW_MODE=sandcastle` with `execution.mode: multica` — sandcastle wins). This is enforced by `resolveMode` tier ordering.
...
INFO [design-review-dispatch] mode={mode_decision} source={source} epic_override={field_sources.execution_runtime.epic_override|null}
```
...
>
> **Scope reminder:** the gate inspects only the depth-0 `unblocked_stories[]` set passed to this skill call. Stories at later dependency depths are gated on their own subsequent dispatch tick when `/design-review` re-enters this skill for the next peer set. See [`hive/references/parallel-call-sites.md`](../../../hive/references/parallel-call-sites.md) for the catalog of dispatch points subject to this gate.
...
This skill is the single dispatch point for `/design-review` mode selection, the parallel-dispatch gate (Step 1.5, `ed-7`), and the executor-vs-orchestrator runner cutover for design-review workflows. Callers must consume `mode_decision`, `mode_reason`, `gate_violations[]`, `runner_path`, and `runner_reason` from this skill instead of re-implementing any of those decisions in another skill or workflow step.

When `mode_decision` resolves to `multica`, route to `skills/hive/skills/design-review-mode-multica/SKILL.md`. When `mode_decision` resolves to `cc-workflows`, route to `skills/hive/skills/design-review-mode-cc-workflows/SKILL.md`. Both atoms ship in later slices — references by skill-path here serve as forward declarations; their absence does not break this dispatch skill.

The parallel-dispatch gate is reachable from no other surface: any future skill that wants to fan design-review stories out concurrently MUST do so through this dispatch point so the gate inspects its `unblocked_stories[]` set, and MUST add a row to [`hive/references/parallel-call-sites.md`](../../../hive/references/parallel-call-sites.md) §2 for the new dispatch shape.
