...
Called once per parent workflow when `mode_decision == multica` AND `multica_submode == cell` was resolved by the step-6e flag shim. The trigger is either:

- `HIVE_EXECUTION_MODE=multica` with `HIVE_MULTICA_SUBMODE=cell`
- root `hive.config.yaml` with `execution.mode: multica` and `execution.multica.submode: cell`
...
const maxWallClockMs = (hive_config?.execution?.multica?.story_timeout_seconds ?? 2700) * 1000;
...
#### 3c-post. Push-target verification (H3 enforcement)

After `pollTaskUntilTerminal` returns and **before** writing the episode marker, verify the child agent obeyed the push-target constraint:

```js
import { verifyPushTarget } from '../../../../hive/lib/multica-story-dispatch/index.mjs';

const pushCheck = await verifyPushTarget(terminal.task_id, epicHandle);
if (pushCheck.orphanPushExists) {
  // H3: orphan-branch push detected — workflow-phase outcome is `failed`, NOT `escalated`.
  phaseOutcome = 'failed';
  phaseNotes = `H3 violation: agent pushed to agent/developer/${terminal.task_id}`;
  // apply max_step_retries retry logic per U2 core_phase_fail, then surface failure
}
// No orphan push → continue to step 3d normally.
```
...
`verifyPushTarget` throws `TRANSPORT` if `git ls-remote` cannot reach the remote; treat this the same as a `TRANSPORT` poll failure (3-strike rule from episode-sync applies).
...
| `circuit_breaker_hit` | `terminal.status === 'cancelled'` (wall-clock timeout — `story_timeout_seconds`) | `fail_story` — cell terminated; mark parent `failed`; markers reflect final state |
...
- H3 orphan-branch push: `verifyPushTarget` returns `orphanPushExists=true`; treated as `core_phase_fail`; retry then `fail_story` if exhausted.
...
execution:
  ...
  multica:
    submode: cell                     # cell (new) | flat (legacy); default cell post-tce-15
...
- `hive/lib/multica-story-dispatch/index.mjs` (tce-12) — `serializeStoryBrief`, `resolveAgentUuidByName`, `verifyPushTarget`.
...
| Per-workflow-phase episode marker | `writeMulticaRunEpisode({phase})` called after each child terminates (step 3d) |
| H3 push verification before marker write | `verifyPushTarget` called in step 3c-post; orphan push → `failed`, never `escalated` |
