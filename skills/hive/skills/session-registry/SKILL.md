...
skill ensures `${HIVE_STATE_DIR}/sessions/index.yaml` exists and is in the correct format. Run once
per epic execution — idempotent if the file already exists.

**Input:** Called by the orchestrator at the start of step 6c in `skills/execute/SKILL.md`.
No `$ARGUMENTS` required.

## When to Use

- At the start of any session-based epic execution (step 6c path in execute skill)
...
### 1. Check if `${HIVE_STATE_DIR}/sessions/` exists

```
stat ${HIVE_STATE_DIR}/sessions/
```

If the directory does NOT exist: create it.

### 2. Check if `${HIVE_STATE_DIR}/sessions/index.yaml` exists

Read `${HIVE_STATE_DIR}/sessions/index.yaml`.
...
Write `${HIVE_STATE_DIR}/sessions/index.yaml` with this content (substituting the current
...
Log `SESSION REGISTRY: initialized at ${HIVE_STATE_DIR}/sessions/index.yaml`.

### 4. Report

Report one line:
```
SESSION REGISTRY: ready — ${HIVE_STATE_DIR}/sessions/index.yaml ({N} sessions)
```
where N is the count of existing sessions (0 for a fresh init).

## Adding a Session Record

After the registry is bootstrapped, the execute skill (step 6c) adds session records
as it creates sessions. Use this append pattern:

1. Read `${HIVE_STATE_DIR}/sessions/index.yaml`
...
4. **One index per project.** `${HIVE_STATE_DIR}/sessions/index.yaml` is the single source of truth.
...
- `skills/execute/SKILL.md` — calls this skill at start of step 6c
