...
- Specific effort-estimator signals and thresholds (decision #2 above named the approach, not the thresholds).
- Shape of the forced user confirmation gate after design discussion — TUI prompt, required sign-off artifact, or both.

---

## 9. Suggested Slicing Hint (for planner)

Vertical slice candidate for first milestone: **one epic, one planning run, lite mode end-to-end, with a visually rich design discussion (including the forced user confirmation gate) and structured outline.** That proves Workstream A (lite path + gate) + Workstream D (rich outputs) together on a single thread before Workstream B (respawn lifecycle + memory bridging — coordinated with the Memory & Autonomous Execution foundation) and Workstream C (model tiering) land.
